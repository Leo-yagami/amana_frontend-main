...
export default function StatsSection() {
      ...
      (context) => {
        ...
        gsap.to(track, {
          ...
          scrollTrigger: {
            ...
            pin: true,
            pinSpacing: true,
            scrub: isMobile ? 0.6 : true, // Added light inertia smoothing on touch devices
            fastScrollEnd: true,
            preventOverlaps: true,
            invalidateOnRefresh: true,
            anticipatePin: 1,
            snap: {
              snapTo: STATS.length > 1 ? 1 / (STATS.length - 1) : 1,
            ...
            onLeaveBack: () => window.dispatchEvent(new CustomEvent("force-hide-nav", { detail: { hidden: false } })),
            onUpdate: (self) => {
              // Code Grid card index sync math
              const idx = Math.min(
  ...
  return (
    <div
      ref={wrapperRef}
      className="relative w-full h-svh bg-secondary/40 border-y border-border overflow-hidden touch-pan-y overscroll-x-none"
    >
      <div
        ref={trackRef}
      ...
      >
        {STATS.map((stat, i) => (
          <div
            key={stat.labelKey}
            className="flex-shrink-0 w-screen h-full flex flex-col items-center justify-center text-center px-8 relative select-none"
          >
            {i !== STATS.length - 1 && (
              <span className="absolute right-0 top-1/4 h-1/2 w-px bg-border pointer-events-none" />
            )}
            <div className="font-display font-extrabold tracking-tight text-[clamp(3.5rem,10vw,8.5rem)] leading-none mb-4">
              {stat.value}
            </div>
      ...
      <div className="flex absolute bottom-[calc(2rem+env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 items-center gap-3 font-mono text-xs uppercase tracking-[0.15em] text-muted-foreground z-10 pointer-events-none">
        <span ref={labelRef}>01 / 0{STATS.length}</span>
        <span className="w-28 h-0.5 bg-border rounded-full overflow-hidden">
          <span
            ref={barRef}
            className="block h-full bg-primary rounded-full transition-all duration-75"
            style={{ width: "0%" }}
          />
        </span>
      </div>
