...
//     if (!gl) {
//       console.error("[shader] Failed to acquire WebGL context — canvas will stay blank");
//       return;
//     }

//     // Surfacing compile/link failures loudly instead of console.warn-and-
//     // continue. Previously a failed compile would warn but linkProgram()
//     // still ran on a broken/incomplete program with no status check at
//     // all — on some mobile drivers that silently produces a program that
//     // "links" but draws nothing, which is exactly the blank-canvas
//     // symptom with zero clue why. This throws instead, so the real error
//     // (which line, what GLSL construct) shows up in the console the
//     // moment it happens on-device.
//     const compile = (type: number, src: string) => {
//       const sh = gl.createShader(type)!;
//       gl.shaderSource(sh, src);
//       gl.compileShader(sh);
...
//         console.error(`[shader compile error: ${kind}]`, log);
...
//     // Flat 0.35 scale everywhere — matches the original desktop version
//     // exactly. Tiering by dpr/screen width isn't the fix for the
//     // aliasing complaint (desktop always ran at 0.35 too), so keep this
//     // simple until the compile/render issue above is confirmed fixed.
//     const getScale = () => 0.35;
...
// // 4 octaves is stable even on mobile.
// float fbm(vec2 p) {
//   float v = 0.0;
//   float a = 1.0;
...
// FBM with 5 octaves — normalize by 1.9375 (1 + 0.5 + 0.25 + 0.125 + 0.0625)
// Unrolled rotation matrix to avoid mat2 overhead. With highp precision,
// 5 octaves is stable on mobile and gives richer, finer detail.
float fbm(vec2 p) {
  float v = 0.0;
  float a = 1.0;
