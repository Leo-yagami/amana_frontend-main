...
function parseHsl(hslStr: string): [number, number, number] {
  if (!hslStr) return [0.1, 0.5, 0.5]; // Safe default fallback

  // Clean string: strip hsl(), hsla(), %, deg, and excess punctuation
  const cleanStr = hslStr
    .replace(/hsla?\(/gi, "")
    .replace(/\)/g, "")
    .replace(/%/g, "")
    .replace(/deg/g, "")
  ...
  // If parsing fails, return a safe vibrant default rather than [0, 0, 0] (black)
  if (isNaN(h) || isNaN(s) || isNaN(l)) return [0.1, 0.5, 0.5];
...
void main() {
  ...
  float zoom = res.x < res.y ? 1.5 : 1.0;
  st *= zoom;
  st -= 0.5 * vec2((res.x / res.y) * zoom, zoom);
  ...
  // ── Dark mode ──
  vec3 colDark = mix(u_c1 * 0.65, u_c1 * 1.25, f) + u_c2 * pow(f, 3.5) * 0.45;
  float alphaDark = 0.32 * f + 0.08;

  // ── Light mode ──
  vec3 colLight = mix(u_c1 * 1.8, u_c1 * 2.4, fc) + u_c2 * pow(fc, 2.5) * 0.4;
  float alphaLight = 0.28 * mix(0.4, 1.0, fc);
...
function useShaderBackground(
  ...
  useEffect(() => {
    ...
    // Context fallback with proper reassignment
    ...
    const resize = () => {
      ...
      canvas.width = Math.max(1, Math.floor(clientW * SCALE));
      canvas.height = Math.max(1, Math.floor(clientH * SCALE));
      gl.viewport(0, 0, canvas.width, canvas.height);
    };
    ...
    let cachedDark = isDarkMode();
    let hasFoundValidColors = false;
    let cachedC1: [number, number, number] = cachedDark ? [0.1, 0.7, 0.7] : [0.2, 0.2, 0.25];
    let cachedC2: [number, number, number] = cachedDark ? [0.9, 0.6, 0.2] : [0.5, 0.5, 0.5];
    ...
    const updateColors = () => {
      ...
      if (rawC1 || rawC2) {
        hasFoundValidColors = true;
        if (rawC1) cachedC1 = parseHsl(rawC1);
        if (rawC2) cachedC2 = parseHsl(rawC2);
      }
    };
    ...
    const render = (now: number) => {
      ...
      const expectedW = Math.max(1, Math.floor((canvas.clientWidth || window.innerWidth) * getScale()));
      const expectedH = Math.max(1, Math.floor((canvas.clientHeight || window.innerHeight) * getScale()));
      if (canvas.width !== expectedW || canvas.height !== expectedH) {
        resize();
      }
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ width: "max-content" }}
        >
          <div ref={tickerInnerRef} className="flex items-center gap-0 shrink-0">
            {TICKER_ITEMS.map((item, i) => (
              <div
                key={i}
                ref={(el) => {
