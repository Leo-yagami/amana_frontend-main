...
const fragmentShader = `
  ...
  // Voronoi-based cell noise — sophisticated, organic, feels designed not generated
  float hash(vec2 p) {
  ...
  // Voronoi pattern — creates flowing cellular structure
  vec2 voronoi(vec2 x) {
    vec2 n = floor(x);
    vec2 f = fract(x);
    float md = 8.0;
    ...
    for (int j = -1; j <= 1; j++) {
      for (int i = -1; i <= 1; i++) {
        ...
        vec2 o = hash(n + g) * vec2(0.5) + vec2(0.25); // Smooth cell centers
        vec2 r = g + o - f;
        float d = dot(r, r);
        if (d < md) {
          md = d;
