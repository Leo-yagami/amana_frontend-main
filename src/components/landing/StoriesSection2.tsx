...
export default function StoriesSection() {
    ...
    () => {
      ...
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: wrapperEl,
          start: "top top",
          end: () => `+=${window.innerHeight * 1.5}`, // Reverted back to original
          pin: true,
          pinSpacing: true,
          scrub: 0.5,
          invalidateOnRefresh: true,
      ...
      cardWrappers.forEach((cardWrapper, i) => {
        ...
        // 1. Slide current card up into its stacked position (Reverted back to tl.to)
        tl.to(
          cardWrapper,
          {
            y: PIN_STEP_PX * i,
  ...
  return (
    <section id="stories" ref={sectionRef} className="relative">
      <div
        ref={wrapperRef}
        // Retained only the vertical centering CSS for mobile
        className="relative container mx-auto px-4 max-w-2xl min-h-[100svh] sm:min-h-screen flex flex-col justify-center sm:justify-start pb-10 sm:pb-24 lg:pb-32"
      >
        {/* Header: Retained mobile padding tweaks */}
        <div className="pt-6 sm:pt-24 lg:pt-28 pb-6 sm:pb-8">
          <SectionHeading
            align="center"
            kicker={t("stories.kicker", "In Their Words")}
