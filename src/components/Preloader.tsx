...
export default function Preloader() {
  ...
  useEffect(() => {
    ...
    const initialBlur = isMobile ? "blur(0px)" : "blur(6px)";
    
    // Dynamic path lengths for outer (r=270) and inner (r=210)
    const outerLen = signalOuterRef.current?.getTotalLength() || 2 * Math.PI * 270;
    const innerLen = signalInnerRef.current?.getTotalLength() || 2 * Math.PI * 210;
    ...
    tl.to(signalOuterRef.current, {
      strokeDashoffset: 0,
      duration: dur(1.4),
      ease: "power2.inOut",
    }, 0.3)
      .to(signalInnerRef.current, {
        strokeDashoffset: 0,
        duration: dur(1.2),
        ease: "power2.inOut",
      }, 0.45);
  ...
  return (
    ...
    >
      ...
      <svg ref={cornerTLRef} className="absolute top-6 left-6 md:mix-blend-overlay" width="22" height="22" viewBox="0 0 28 28" aria-hidden="true">
        <path d="M 0 28 L 0 0 L 28 0" fill="none" stroke="currentColor" strokeWidth="1" className="text-foreground" />
      </svg>
      <svg ref={cornerBRRef} className="absolute bottom-6 right-6 md:mix-blend-overlay" width="22" height="22" viewBox="0 0 28 28" aria-hidden="true">
        <path d="M 28 0 L 28 28 L 0 28" fill="none" stroke="currentColor" strokeWidth="1" className="text-foreground" />
      </svg>

      {/* Expanded SVG container framing around the text */}
      <div className="absolute inset-0 w-full h-full pointer-events-none flex items-center justify-center">
        <svg
          className="w-[90vw] max-w-[480px] aspect-square overflow-visible"
          viewBox="0 0 600 600"
          aria-hidden="true"
        >
          {/* <circle ref={signalOuterRef} cx="50%" cy="50%" r="280" fill="none" stroke="hsl(var(--primary))" strokeWidth="1" />
         <circle ref={signalInnerRef} cx="50%" cy="50%" r="190" fill="none" stroke="hsl(var(--primary))" strokeWidth="1" /> */}
          <circle
            ref={signalOuterRef}
            cx="300"
            cy="300"
